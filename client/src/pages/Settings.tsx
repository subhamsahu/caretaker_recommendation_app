...
export default function Settings() {
  ...
  // Content tab — persisted in localStorage
  const [safetyLevel, setSafetyLevel] = useState<string>(
    () => localStorage.getItem('safetyLevel') ?? 'strict'
  )
  const [hiddenCategories, setHiddenCategories] = useState<string[]>(
    () => JSON.parse(localStorage.getItem('hiddenCategories') ?? '[]')
  )
  const [screenTimeAlert, setScreenTimeAlert] = useState<boolean>(
    () => localStorage.getItem('screenTimeAlert') !== 'false'
  )
  const [weeklyReport, setWeeklyReport] = useState<boolean>(
    () => localStorage.getItem('weeklyReport') !== 'false'
  )
  ...
  const toggleHiddenCategory = (cat: string) => {
  ...
  const handleSafetyChange = (value: string) => {
    ...
    localStorage.setItem('safetyLevel', value)
  }

  const handleScreenTimeAlert = (v: boolean) => {
    setScreenTimeAlert(v)
    localStorage.setItem('screenTimeAlert', String(v))
  }

  const handleWeeklyReport = (v: boolean) => {
    setWeeklyReport(v)
    localStorage.setItem('weeklyReport', String(v))
  }
  ...
  const handleResetSettings = () => {
    ...
    localStorage.removeItem('safetyLevel')
    localStorage.removeItem('hiddenCategories')
    localStorage.removeItem('screenTimeAlert')
                  ...
                  <div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-2">
                      {([
                        ...
                        {
                          id: 'rose-dark' as Theme,
                          label: '🌸 Rose Dark',
                          swatches: ['#2e1420', '#3c1a2a', '#e91e8c'],
                        },
                        {
              ...
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div>
                    ...
                    <Input type="email" value={userEmail} readOnly className="bg-muted cursor-default" />
                  </div>
            ...
            <div className="space-y-6">
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Control what kinds of videos appear in your children's recommendations
                  </CardDescription>
                </CardHeader>
                <CardContent className="space-y-5">
                  <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      <p className="font-medium">Safety filter</p>
                      <p className="text-sm text-muted-foreground">Minimum safety threshold for recommended videos</p>
                    </div>
                    <select
                      value={safetyLevel}
                    ...
                    >
                      ...
                      <option value="balanced">Balanced (≥ 0.85)</option>
                      <option value="relaxed">Relaxed (≥ 0.70)</option>
                    </select>
                  </div>
                  ...
                  <div>
                    <p className="font-medium mb-1">Hidden categories</p>
                    <p className="text-sm text-muted-foreground mb-3">
                      Checked categories will never appear in recommendations
                    </p>
                      ...
                      {INTEREST_OPTIONS.map((cat) => (
                        <label key={cat} className="flex items-center gap-2 cursor-pointer select-none">
                          <input
                            ...
                            onChange={() => toggleHiddenCategory(cat)}
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Warn when a child reaches their screen time limit</p>
                    </div>
                    <input
                      type="checkbox"
                      checked={screenTimeAlert}
                      onChange={(e) => handleScreenTimeAlert(e.target.checked)}
                  ...
                  <div className="flex items-center justify-between gap-4">
                    <div>
                      <p className="font-medium">Weekly watch summary</p>
                      <p className="text-sm text-muted-foreground">Show a weekly summary on the dashboard</p>
                    </div>
                    <input
                      type="checkbox"
                      checked={weeklyReport}
                      onChange={(e) => handleWeeklyReport(e.target.checked)}
                      className="h-5 w-5 shrink-0 accent-primary cursor-pointer"
                    />
                  </div>
                </CardContent>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>
                    Add or manage your children's profiles, ages, and interests
                  </CardDescription>
                </CardHeader>
                <CardContent>
                  <div className="flex flex-col gap-2 sm:flex-row">
                    <Button variant="outline" className="w-full sm:w-auto" onClick={() => navigate('/profiles')}>
                      View Profiles
                    </Button>
                    <Button className="w-full sm:w-auto" onClick={() => navigate('/add-child')}>
                      Add Child
                    </Button>
                  </div>
                </CardContent>
                ...
                <CardContent className="space-y-5">
                  ...
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Sign out of your KidCare account</p>
                    </div>
                ...
                <CardContent className="space-y-4">
                  <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Restore theme, content, and display preferences to defaults</p>
                    </div>
